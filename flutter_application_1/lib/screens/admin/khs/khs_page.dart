...
import 'package:flutter_application_1/screens/admin/khs/khs_kelas_page.dart';
import 'package:flutter_application_1/widgets/admin/custom_bottom_nav.dart';
import 'package:flutter_application_1/models/admin/khs_model.dart';
import 'package:flutter_application_1/services/admin/khs_service.dart';
...
class _KhsPageState extends State<KhsPage> {
  ...
  List<KhsModel> dataKhs = [];
  bool isLoading = true;

  final KhsService khsService = KhsService();
  ...
  Future<void> loadData() async {
    try {
      final data = await khsService.getKhs();
      setState(() {
        dataKhs = data;
        isLoading = false;
      });
  ...
  Widget build(BuildContext context) {
    ...
    final List<KhsModel> displayedKhs = isExpanded 
        ? dataKhs 
        : dataKhs.take(5).toList();

    // Cek apakah data aslinya lebih dari 5 item untuk menentukan apakah tombol "Lainnya" perlu ditampilkan
    final bool showLainnyaButton = dataKhs.length > 5;
            ...
            Container(
              ...
              child: Column(
                children: [
                  ...
                  ...displayedKhs.map((item) => _buildRow(item)),
 ...
 Widget _buildRow(KhsModel item) {
  return InkWell(
    onTap: () {
      Navigator.push(
        context,
        MaterialPageRoute(
          builder: (_) => KhsKelasPage(
